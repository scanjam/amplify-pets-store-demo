...
export default function PetCreateForm(props) {
  ...
  const [ag, setAg] = React.useState(initialValues.ag);
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.ag ?? value;
          }
          if (errors.ag?.hasError) {
            runValidationTasks("ag", value);
          }
          setAg(value);
        }}
        onBlur={() => runValidationTasks("ag", ag)}
        errorMessage={errors.ag?.errorMessage}
        hasError={errors.ag?.hasError}
        {...getOverrideProps(overrides, "ag")}
      ></TextField>
      <TextField
        label="Breed"
        isRequired={false}
